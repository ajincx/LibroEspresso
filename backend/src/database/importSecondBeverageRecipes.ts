...
async function resolveBatch(client: pg.PoolClient): Promise<ResolvedRecipe[]> {
  ...
  for (const candidate of SECOND_BEVERAGE_RECIPE_BATCH) {
    ...
    for (const requested of candidate.ingredients) {
      ...
      requireCondition(saved.branchCosts === 1, `${requested.sku} must have one consistent branch cost`);
    ...
    requireCondition(ingredients.some(({ sku }) => sku === "ING-00018"), `${candidate.product} ${candidate.variant} must use Ice By Weight`);
    ...
    resolved.push({ productId: variantRow.productId, variantId: variantRow.variantId, product: candidate.product,
      variant: candidate.variant, sellingPrice, ingredients, recipeCost, margin, marginRate: (margin / sellingPrice) * 100 });
  }
  return resolved;
}
...
async function main() {
  ...
  requireCondition(args.length <= 1 && (args.length === 0 || args[0] === "--apply"),
    "Usage: tsx src/database/importSecondBeverageRecipes.ts [--apply]");
  ...
  try {
    ...
    requireCondition(before.products === 69 && before.variants === 81 && before.ingredients === 36 &&
      before.recipes === 5 && before.recipeItems === 16 && before.sources === 0 && before.mappings === 0,
    ...
    const rm004ReferencesHash = await hashQuery(client,
      `SELECT 'balance' source,to_jsonb(x) data FROM branch_inventory_balances x WHERE inventory_item_id=(SELECT id FROM inventory_items WHERE sku='RM-004')
       UNION ALL SELECT 'setting',to_jsonb(x) FROM branch_inventory_settings x WHERE inventory_item_id=(SELECT id FROM inventory_items WHERE sku='RM-004')
       UNION ALL SELECT 'count',to_jsonb(x) FROM inventory_count_items x WHERE inventory_item_id=(SELECT id FROM inventory_items WHERE sku='RM-004')`);
    ...
    requireCondition(batch.length === 8 && batch.reduce((sum, recipe) => sum + recipe.ingredients.length, 0) === 34,
      "Resolved batch does not contain exactly 8 recipes and 34 recipe items");

    if (!apply) {
      await client.query("ROLLBACK");
      transactionOpen = false;
      console.log(JSON.stringify({ transaction: "DRY_RUN_ROLLED_BACK", before, expectedAfter: { ...before, recipes: 13, recipeItems: 50 },
        basis: SAMPLE_RECIPE_BASIS, recipes: batch }, null, 2));
      return;
    }
    ...
    for (const recipe of batch) {
      const saved = await saveRecipeDefinition(client, {
        ...
        changeReason: SAMPLE_RECIPE_BASIS, createdBy: owner.rows[0]!.id,
      });
      ...
      await client.query(
        ...
        [owner.rows[0]!.id, saved.recipeId, `Created ${recipe.product} ${recipe.variant} sample recipe; ${SAMPLE_RECIPE_BASIS}`,
          JSON.stringify({ source: "secondBeverageRecipeBatch.ts", quantityBasis: SAMPLE_RECIPE_BASIS,
            category: "Cold Classics", product: recipe.product, variant: recipe.variant, recipeCost: recipe.recipeCost,
    ...
    requireCondition(await hashQuery(client, `SELECT * FROM recipes WHERE id=ANY($1::uuid[])`, [existingRecipeIds]) === existingRecipesHash,
      "An existing Standard recipe changed");
    requireCondition(await hashQuery(client, `SELECT * FROM recipe_items WHERE recipe_id=ANY($1::uuid[])`, [existingRecipeIds]) === existingRecipeItemsHash,
      "An existing Standard recipe item changed");
    requireCondition(await hashQuery(client, `SELECT * FROM inventory_items WHERE sku='RM-004'`) === rm004Hash, "RM-004 changed");
    requireCondition(await hashQuery(client,
      `SELECT 'balance' source,to_jsonb(x) data FROM branch_inventory_balances x WHERE inventory_item_id=(SELECT id FROM inventory_items WHERE sku='RM-004')
       UNION ALL SELECT 'setting',to_jsonb(x) FROM branch_inventory_settings x WHERE inventory_item_id=(SELECT id FROM inventory_items WHERE sku='RM-004')
       UNION ALL SELECT 'count',to_jsonb(x) FROM inventory_count_items x WHERE inventory_item_id=(SELECT id FROM inventory_items WHERE sku='RM-004')`) === rm004ReferencesHash,
    "RM-004 balances, settings, or history changed");
    ...
    requireCondition(after.products === 69 && after.variants === 81 && after.ingredients === 36 && after.recipes === 13 &&
      after.recipeItems === 50 && after.sources === 0 && after.mappings === 0, `Unexpected final state: ${JSON.stringify(after)}`);
    const stored = await client.query<{ recipeId: string; variantId: string; version: number; changeReason: string; itemCount: number; usesWeightedIce: boolean; usesRm004: boolean }>(
      `SELECT r.id "recipeId",r.menu_item_variant_id "variantId",r.version,r.change_reason "changeReason",count(ri.id)::int "itemCount",
              bool_or(ii.sku='ING-00018') "usesWeightedIce",bool_or(ii.sku='RM-004') "usesRm004"
    ...
    requireCondition(stored.rows.length === 8 && stored.rows.every((row) => row.version === 1 &&
      row.changeReason === SAMPLE_RECIPE_BASIS && row.usesWeightedIce && !row.usesRm004),
    "Stored recipes failed version, provenance, or Ice identity verification");

    await client.query("COMMIT");
    transactionOpen = false;
    console.log(JSON.stringify({ transaction: "COMMITTED", before, after, basis: SAMPLE_RECIPE_BASIS, inserted,
      existingStandardRecipesVerified: existingRecipeIds.length, protectedTablesVerified: protectedTables }, null, 2));
  } catch (error) {
