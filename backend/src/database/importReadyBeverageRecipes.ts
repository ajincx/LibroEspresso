...
type ResolvedRecipe = {
  productId: string; variantId: string; product: string; variant: string; sellingPrice: number;
...
async function resolveBatch(client: pg.PoolClient): Promise<ResolvedRecipe[]> {
  ...
  for (const candidate of READY_BEVERAGE_RECIPE_BATCH) {
    const variant = await client.query<{ productId: string; variantId: string; sellingPrice: number }>(
      `SELECT mi.id "productId",v.id "variantId",v.selling_price::float8 "sellingPrice"
        ...
        WHERE mc.name=$1 AND mi.name=$2 AND v.name=$3 AND mi.status='ACTIVE' AND mi.approval_status='APPROVED' AND v.status='ACTIVE'`,
      [candidate.category, candidate.product, candidate.variant],
    );
    requireCondition(variant.rows.length === 1, `Expected one active ${candidate.product} Standard variant`);
    ...
    requireCondition(existing.rows.length === 0, `${candidate.product} already has a recipe`);
...
async function main() {
  ...
  requireCondition(args.length <= 1 && (args.length === 0 || args[0] === "--apply"),
    "Usage: tsx src/database/importReadyBeverageRecipes.ts [--apply]");
  ...
  try {
    ...
    requireCondition(before.products === 69 && before.variants === 81 && before.ingredients === 36 &&
      before.recipes === READY_BEVERAGE_BASELINE.recipes && before.recipeItems === READY_BEVERAGE_BASELINE.recipeItems &&
      before.sources === 0 && before.mappings === 0, `Unexpected database state: ${JSON.stringify(before)}`);
    ...
    requireCondition(batch.length === 4 && batch.reduce((sum, recipe) => sum + recipe.ingredients.length, 0) === 8,
      "Resolved batch must contain exactly 4 recipes and 8 recipe items");

    if (!apply) {
      await client.query("ROLLBACK");
      transactionOpen = false;
      console.log(JSON.stringify({ transaction: "DRY_RUN_ROLLED_BACK", before,
        expectedAfter: { ...before, recipes: 36, recipeItems: 141 }, provenance: READY_BEVERAGE_PROVENANCE, recipes: batch }, null, 2));
      return;
    }
    ...
    for (const recipe of batch) {
      const saved = await saveRecipeDefinition(client, {
        ...
        changeReason: READY_BEVERAGE_PROVENANCE, createdBy: owner.rows[0]!.id,
      });
      ...
      await client.query(
        ...
        [owner.rows[0]!.id, saved.recipeId, `Created ${recipe.product} Standard recipe; ${READY_BEVERAGE_PROVENANCE}`,
          JSON.stringify({ source: "readyBeverageRecipeBatch.ts", provenance: READY_BEVERAGE_PROVENANCE,
    ...
    const blocked = await client.query<{ count: number }>(
      `SELECT count(*)::int count FROM recipes r JOIN menu_items mi ON mi.id=r.menu_item_id
        WHERE mi.name IN ('Iced Tea','Bottled Water','Morpheus Pond','Midsummer Sangria','She-a-Frooty','Dairy Dose of Coco','Libro Mood Mover','The Other Choice')`,
    );
    requireCondition(blocked.rows[0]!.count === 0, "A blocked product received a recipe");
    ...
    requireCondition(after.products === 69 && after.variants === 81 && after.ingredients === 36 && after.recipes === 36 &&
      after.recipeItems === 141 && after.sources === 0 && after.mappings === 0, `Unexpected final state: ${JSON.stringify(after)}`);
    const stored = await client.query<{ version: number; changeReason: string; itemCount: number }>(
      `SELECT r.version,r.change_reason "changeReason",count(ri.id)::int "itemCount" FROM recipes r
         JOIN recipe_items ri ON ri.recipe_id=r.id WHERE r.id=ANY($1::uuid[]) GROUP BY r.id`,
      [inserted.map(({ recipeId }) => recipeId)],
    );
    requireCondition(stored.rows.length === 4 && stored.rows.every((row) => row.version === 1 &&
      row.changeReason === READY_BEVERAGE_PROVENANCE && row.itemCount === 2), "Stored recipes failed validation");
    await client.query("COMMIT");
    transactionOpen = false;
    console.log(JSON.stringify({ transaction: "COMMITTED", before, after, provenance: READY_BEVERAGE_PROVENANCE,
      inserted, existingRecipesVerified: existingRecipeIds.length, blockedRecipesVerified: blocked.rows[0]!.count,
