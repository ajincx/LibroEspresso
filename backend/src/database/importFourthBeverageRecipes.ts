...
type ResolvedRecipe = {
  productId: string; variantId: string; category: string; product: string; variant: string; sellingPrice: number;
...
async function resolveBatch(client: pg.PoolClient): Promise<ResolvedRecipe[]> {
  ...
  for (const candidate of FOURTH_BEVERAGE_RECIPE_BATCH) {
    ...
    requireCondition(variant.rows.length === 1, `Expected one active ${candidate.category} / ${candidate.product} / ${candidate.variant}`);
    ...
    requireCondition(ingredients.some(({ sku }) => sku === "ING-00018"), `${candidate.product} ${candidate.variant} must use weighted Ice`);
    requireCondition(!ingredients.some(({ sku }) => sku === "RM-004"), `${candidate.product} ${candidate.variant} cannot use RM-004`);
...
async function main() {
  ...
  requireCondition(args.length <= 1 && (args.length === 0 || args[0] === "--apply"),
    "Usage: tsx src/database/importFourthBeverageRecipes.ts [--apply]");
  ...
  try {
    ...
    requireCondition(before.products === 69 && before.variants === 81 && before.ingredients === 36 &&
      before.recipes === BATCH_FOUR_EXPECTED_BASELINE.recipes && before.recipeItems === BATCH_FOUR_EXPECTED_BASELINE.recipeItems &&
    ...
    requireCondition(batch.length === 11 && batch.reduce((sum, recipe) => sum + recipe.ingredients.length, 0) === 51,
      "Resolved batch must contain exactly 11 recipes and 51 recipe items");

    if (!apply) {
      await client.query("ROLLBACK");
      transactionOpen = false;
      console.log(JSON.stringify({ transaction: "DRY_RUN_ROLLED_BACK", before,
        expectedAfter: { ...before, recipes: 32, recipeItems: 133 }, provenance: BATCH_FOUR_PROVENANCE, recipes: batch }, null, 2));
      return;
    }

    const inserted = [];
    for (const recipe of batch) {
      const saltedCaramelSample = recipe.product === "Salted Caramel" || recipe.product === "Salted Caramel Latte";
      const saved = await saveRecipeDefinition(client, {
        ...
        changeReason: BATCH_FOUR_PROVENANCE, createdBy: owner.rows[0]!.id,
      });
      ...
      await client.query(
        ...
        [owner.rows[0]!.id, saved.recipeId, `Created ${recipe.product} ${recipe.variant} recipe; ${BATCH_FOUR_PROVENANCE}`,
          JSON.stringify({ source: "fourthBeverageRecipeBatch.ts", provenance: BATCH_FOUR_PROVENANCE,
            sampleComposition: saltedCaramelSample ? "Caramel Syrup + Salt demonstration composition" : undefined,
    ...
    requireCondition(after.products === 69 && after.variants === 81 && after.ingredients === 36 && after.recipes === 32 &&
      after.recipeItems === 133 && after.sources === 0 && after.mappings === 0, `Unexpected final state: ${JSON.stringify(after)}`);
    const stored = await client.query<{ recipeId: string; version: number; changeReason: string; usesWeightedIce: boolean; usesRm004: boolean }>(
      `SELECT r.id "recipeId",r.version,r.change_reason "changeReason",bool_or(ii.sku='ING-00018') "usesWeightedIce",bool_or(ii.sku='RM-004') "usesRm004"
    ...
    requireCondition(stored.rows.length === 11 && stored.rows.every((row) => row.version === 1 &&
      row.changeReason === BATCH_FOUR_PROVENANCE && row.usesWeightedIce && !row.usesRm004),
    "Stored recipes failed version, provenance, or Ice verification");
    await client.query("COMMIT");
    transactionOpen = false;
    console.log(JSON.stringify({ transaction: "COMMITTED", before, after, provenance: BATCH_FOUR_PROVENANCE,
