...
const selection = `SELECT u.id,u.branch_id "branchId",u.first_name "firstName",u.last_name "lastName",u.email,u.username,u.phone_number "phoneNumber",u.position,u.role,u.status,u.last_login_at "lastLoginAt",u.created_at "createdAt",u.updated_at "updatedAt",b.code "branchCode",b.name "branchName" FROM users u LEFT JOIN branches b ON b.id=u.branch_id`;
...
export const updateUser: RequestHandler = async (req, res) => {
  ...
  const current = await pool.query<{ role: "OWNER"|"BRANCH_MANAGER"; branch_id: string|null }>("SELECT role,branch_id FROM users WHERE id=$1", [id]);
  ...
  if ((role === "OWNER" && branchId !== null) || (role === "BRANCH_MANAGER" && !branchId)) {
    throw new AppError(422, "INVALID_BRANCH_ASSIGNMENT", "Owner branch must be empty and Branch Manager branch is required");
  }
  ...
  if (role === "BRANCH_MANAGER") {
    ...
    if (!branch.rows[0]) throw new AppError(422, "INVALID_BRANCH", "Select an active branch for the Branch Manager");
  }
