...
const userFields = z.object({ firstName: z.string().trim().min(1).max(80), lastName: z.string().trim().min(1).max(80), email: z.string().email().max(255), username: z.string().trim().min(3).max(80), role: z.enum(["OWNER", "BRANCH_MANAGER"]), branchId: z.string().uuid().nullable(), status: z.enum(["ACTIVE", "INACTIVE"]).default("ACTIVE") });
